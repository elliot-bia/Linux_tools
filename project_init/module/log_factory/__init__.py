    ...
    _LOG_CONFIG_DICT = {
        ...
        'handlers': {
            ...
            'debug_file_error': {
                'level': 'ERROR',
                'class': 'logging.handlers.RotatingFileHandler',
                'filename': _get_filename(filename='debug_ERROR.log', ),
                'maxBytes': _SINGLE_FILE_MAX_BYTES,  # 2GB
                'encoding': 'UTF-8',
                'backupCount': _BACKUP_COUNT,
                'formatter': 'prod',
            ...
            # 接下来这里就可以自定义了，上面是比较固定的
            'business_info': {
                'level': 'INFO',
                'class': 'logging.handlers.RotatingFileHandler',
                'filename': _get_filename(filedir='business',filename='business_INFO.log'),
                'maxBytes': _SINGLE_FILE_MAX_BYTES,  # 2GB
                'encoding': 'UTF-8',
                'backupCount': _BACKUP_COUNT,
                'formatter': 'prod',
        ...
        'loggers': {
            ...
            'logginglib_project.business_layer': {
                'handlers': ['business_info', 'debug_console'],
                'level': 'INFO',
            }
        },
    }
